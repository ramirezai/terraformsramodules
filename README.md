...
This repository uses a two-layer Terraform architecture:

```
aws/
├── platform/                    # Layer 1: Account-level (AWS + Databricks Account API)
│   ├── main.tf
│   ├── backend.tf
│   └── ...
├── workspace/                   # Layer 2: Workspace-level (Databricks Workspace API)
│   ├── main.tf
│   ├── backend.tf
│   ├── remote_state.tf
│   └── ...
├── modules/                     # Shared modules
├── customizations/              # Optional workspace customizations
└── environments/
    ├── dev/
    │   ├── platform.tfvars
    │   ├── platform-backend.hcl
    │   ├── workspace.tfvars
    │   └── workspace-backend.hcl
    └── prod/
        ├── platform.tfvars
        ├── platform-backend.hcl
        ├── workspace.tfvars
...
2. Install [Terraform](https://developer.hashicorp.com/terraform/downloads) (v1.10+ required for S3 native locking).
3. Create an S3 bucket for Terraform state (enable versioning for state history and recovery).
4. Fill out the backend config files in `environments/<env>/` (e.g., `platform-backend.hcl` and `workspace-backend.hcl`) with your bucket name, key, and region.
5. Fill out the tfvars files in `environments/<env>/` (e.g., `platform.tfvars` and `workspace.tfvars`).
...
7. Deploy the platform layer first:
   ```bash
   cd platform
   terraform init -backend-config=../environments/dev/platform-backend.hcl -reconfigure
   terraform plan -var-file=../environments/dev/platform.tfvars
   terraform apply -var-file=../environments/dev/platform.tfvars
   ```
8. Deploy the workspace layer (after platform succeeds):
   ```bash
   cd workspace
   terraform init -backend-config=../environments/dev/workspace-backend.hcl -reconfigure
   terraform plan -var-file=../environments/dev/workspace.tfvars
   terraform apply -var-file=../environments/dev/workspace.tfvars
   ```
